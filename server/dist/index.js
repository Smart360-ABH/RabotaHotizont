...
const parseClient_1 = require("./parseClient");
...
app.post('/api/disputes', async (req, res) => {
    ...
    try {
        ...
        // 1. Verify User
        const user = await (0, parseClient_1.parseRequest)('/users/me', 'GET', null, sessionToken);
        if (!user || !user.objectId)
            return res.status(401).json({ error: 'Invalid session' });
        // 2. Fetch Order
        const order = await (0, parseClient_1.parseRequest)(`/classes/Order/${orderId}`, 'GET', null, null, true);
        ...
        const existingQuery = qs_1.default.stringify({
            where: JSON.stringify({
                order: { __type: 'Pointer', className: 'Order', objectId: orderId },
                status: { $in: ['opened', 'negotiating', 'escalated'] }
            })
        });
        const existing = await (0, parseClient_1.parseRequest)(`/classes/Dispute?${existingQuery}`, 'GET', null, null, true);
        ...
        const disputePayload = {
            order: { __type: 'Pointer', className: 'Order', objectId: orderId },
            product: order.items && order.items.length === 1 ? { __type: 'Pointer', className: 'Product', objectId: order.items[0].productId } : undefined,
            initiator: { __type: 'Pointer', className: '_User', objectId: user.objectId },
            respondent: { __type: 'Pointer', className: 'Vendor', objectId: order.vendorId },
            reason,
            description,
        ...
        const createdDispute = await (0, parseClient_1.parseRequest)('/classes/Dispute', 'POST', disputePayload, null, true);
...
app.post('/api/reviews', async (req, res) => {
    ...
    try {
        ...
        const orderData = await (0, parseClient_1.parseRequest)(`/classes/Order/${order}`, 'GET', null, null, true); // Use Master Key to safely inspect
        ...
        const hasProduct = orderData.items && orderData.items.some((item) => item.objectId === product || item.productId === product);
        if (!hasProduct) {
        ...
        const result = await (0, parseClient_1.parseRequest)('/classes/Review', 'POST', reviewPayload, null, true); // Master Key creation to enforce schema/CLPs if needed
...
app.get('/api/reviews', async (req, res) => {
    try {
        ...
        const result = await (0, parseClient_1.parseRequest)(`/classes/Review${query}${limit}${order}`, 'GET');
        res.json(result);
    }
    catch (error) {
...
app.put('/api/reviews/:id', requireAdminSecret, async (req, res) => {
    try {
        const result = await (0, parseClient_1.parseRequest)(`/classes/Review/${req.params.id}`, 'PUT', req.body);
        res.json(result);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
app.delete('/api/reviews/:id', requireAdminSecret, async (req, res) => {
    try {
        const result = await (0, parseClient_1.parseRequest)(`/classes/Review/${req.params.id}`, 'DELETE');
        res.json(result);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
app.post('/api/appeals', async (req, res) => {
    try {
        const result = await (0, parseClient_1.parseRequest)('/classes/Appeal', 'POST', req.body, req.header('X-Parse-Session-Token'));
        res.json(result);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
app.get('/api/appeals', requireAdminSecret, async (req, res) => {
    try {
        const query = req.query.where ? `?where=${req.query.where}` : '';
        const result = await (0, parseClient_1.parseRequest)(`/classes/Appeal${query}`, 'GET');
        res.json(result);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
app.put('/api/appeals/:id', requireAdminSecret, async (req, res) => {
    try {
        const result = await (0, parseClient_1.parseRequest)(`/classes/Appeal/${req.params.id}`, 'PUT', req.body);
        res.json(result);
    }
    catch (error) {
    }
...
app.delete('/api/products/:id', async (req, res) => {
    ...
    try {
        ...
        const product = await (0, parseClient_1.parseRequest)(`/classes/Product/${req.params.id}`, 'GET', null, null, true);
        ...
        // 4. Delete (Master Key)
        await (0, parseClient_1.parseRequest)(`/classes/Product/${req.params.id}`, 'DELETE', null, null, true);
...
app.put('/api/orders/:id/status', async (req, res) => {
    ...
    try {
        ...
        const disputeQuery = qs_1.default.stringify({
            where: JSON.stringify({
                order: { __type: 'Pointer', className: 'Order', objectId: orderId },
                status: { $in: ['opened', 'negotiating', 'escalated'] }
            })
        });
        const activeDisputes = await (0, parseClient_1.parseRequest)(`/classes/Dispute?${disputeQuery}`, 'GET', null, null, true);
        ...
        const result = await (0, parseClient_1.parseRequest)(`/classes/Order/${orderId}`, 'PUT', updatePayload, null, true);
        res.json(result);
    }
    catch (error) {
...
app.post('/api/conversations', async (req, res) => {
    ...
    try {
        ...
        if (!IS_PROD && sessionToken.startsWith('mock_session_token_vendor')) {
            ...
            user = await (0, parseClient_1.parseRequest)('/users/me', 'GET', null, sessionToken);
        }
        if (!user || !user.objectId)
            return res.status(401).json({ error: 'Invalid session' });
        ...
        const existing = await (0, parseClient_1.parseRequest)(`/classes/Conversation?${existingQuery}`, 'GET', null, null, true);
        let match = null;
        if (existing.results && existing.results.length > 0) {
            if (context) {
                ...
                match = existing.results.find((c) => JSON.stringify(c.context) === JSON.stringify(context));
            }
            else {
                ...
                match = existing.results.find((c) => !c.context || Object.keys(c.context).length === 0);
            }
        }
        if (match) {
        ...
        const newConv = await (0, parseClient_1.parseRequest)('/classes/Conversation', 'POST', conversationPayload, null, true);
        res.json(newConv);
    }
    catch (error) {
...
app.post('/api/messages', async (req, res) => {
    ...
    try {
        ...
        const conversation = await (0, parseClient_1.parseRequest)(`/classes/Conversation/${conversationId}`, 'GET', null, null, true);
        if (!conversation || conversation.error)
            return res.status(404).json({ error: 'Conversation not found' });
        // Check if user is participant (participants is array of pointers)
        const isParticipant = conversation.participants.some((p) => p.objectId === user.objectId);
        if (!isParticipant)
        ...
        const newMessage = await (0, parseClient_1.parseRequest)('/classes/Message', 'POST', messagePayload, null, true);
        // 4. Update Conversation Timestamp
        await (0, parseClient_1.parseRequest)(`/classes/Conversation/${conversationId}`, 'PUT', {
            lastMessageAt: { __type: 'Date', iso: new Date().toISOString() }
        }, null, true);
        res.json(newMessage);
    }
...
app.get('/api/conversations/:id/messages', async (req, res) => {
    ...
    try {
        ...
        const messages = await (0, parseClient_1.parseRequest)(`/classes/Message?${query}`, 'GET', null, null, true);
        res.json(messages);
    }
    catch (error) {
...
app.get('/api/conversations', async (req, res) => {
    ...
    try {
        ...
        const conversations = await (0, parseClient_1.parseRequest)(`/classes/Conversation?${query}`, 'GET', null, null, true);
...
app.post('/api/audit', async (req, res) => {
    try {
        const result = await (0, parseClient_1.parseRequest)('/classes/AuditLog', 'POST', req.body);
        res.json(result);
    }
    catch (error) {
...
app.get('/api/audit', requireAdminSecret, async (req, res) => {
    try {
        const query = req.query.where ? `?where=${req.query.where}` : '';
        const limit = req.query.limit ? `&limit=${req.query.limit}` : '';
        const result = await (0, parseClient_1.parseRequest)(`/classes/AuditLog${query}${limit}`, 'GET');
        res.json(result);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
