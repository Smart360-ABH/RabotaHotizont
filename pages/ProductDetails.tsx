...
import { Star, Truck, ShieldCheck, Heart, ShoppingCart, Minus, Plus, Check, MessageSquare, Send } from 'lucide-react';
import { useMarket } from '../context/MarketContext';
import { ProductCard } from '../components/ProductCard';

export const ProductDetails: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const { products, addToCart, cart, updateQuantity, toggleFavorite, favorites, reviews, addReview, user, getVendorById } = useMarket();
            ...
            <div className="prose prose-slate dark:prose-invert max-w-none text-gray-600 dark:text-gray-300">
                {activeTab === 'desc' ? (
                    ...
                    <div className="space-y-8">
                        {/* Write Review Form */}
                        {user ? (
                            <div className="bg-gray-50 dark:bg-slate-800 p-6 rounded-xl border dark:border-slate-700">
                                <h4 className="font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                                    <MessageSquare className="w-5 h-5"/> Оставить отзыв
                                </h4>
                                    ...
                                    <div className="mb-4">
                                        <div className="flex gap-2 mb-2">
                                            {[1, 2, 3, 4, 5].map(star => (
                                                <Star 
                                                    key={star}
                                                    className={`w-6 h-6 cursor-pointer transition ${star <= rating ? 'text-yellow-400 fill-current' : 'text-gray-300 dark:text-gray-600'}`}
                                                    onClick={() => setRating(star)}
                                                />
                                            ))}
                                        </div>
                                        <span className="text-sm text-gray-500">Нажмите на звезды для оценки</span>
                                    </div>
                                    <div className="mb-4">
                                        <textarea 
                                            className="w-full p-3 border dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none"
                                            rows={3}
                            ...
                            <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 text-yellow-800 dark:text-yellow-200 rounded-lg text-center">
                                <Link to="/login" className="font-bold underline">Войдите</Link>, чтобы оставить отзыв.
                            </div>
                        )}
                        ...
                        <div className="space-y-6">
                            {productReviews.length > 0 ? productReviews.map(review => (
                                <div key={review.id} className="border-b dark:border-slate-700 pb-6 last:border-0 animate-fade-in">
                                    <div className="flex items-center gap-3 mb-3">
                                        <div className="w-10 h-10 bg-indigo-100 dark:bg-slate-700 rounded-full flex items-center justify-center font-bold text-indigo-600 dark:text-white">
                                            {review.userName[0]}
                                        </div>
                                        <div>
                                        ...
                                        <div className="ml-auto flex text-yellow-400 bg-yellow-50 dark:bg-yellow-900/10 px-2 py-1 rounded-full">
                                            {[...Array(5)].map((_, j) => (
                                                <Star key={j} className={`w-3 h-3 ${j < review.rating ? 'fill-current' : 'text-gray-300 dark:text-gray-600'}`} />
                                            ))}
                                        </div>
                                    </div>
